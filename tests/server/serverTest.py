...
class UpdateServer(Server):
    ...
    def test_10ValidateDuplicateServerName(self):
        response = requests.put(URL,json={'description':'test','hostname':'test12345','name':'test'})
        value=response.json()
        if response.status_code==201:
            print "new server created"
            self.assertEqual(response.status_code, 201)
    ...
    def test_11ValidateDuplicateHostName(self):
        response = requests.put(URL,json={'description':'test','hostname':'test','name':'test12345'})
        value=response.json()
        if response.status_code==201:
            self.assertEqual(response.status_code, 201)
        else:
